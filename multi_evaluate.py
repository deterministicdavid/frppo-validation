...
def run_evaluation_and_process(
    ...
    for conf_path in config_file_list:
        ...
        if common_env_name is None:
            ...
            raise ValueError(
                f"Trying to compare rewards of {common_env_name} and {env_name} which makes no sense."
            )
        ...
        log_dir = log_config.get("log_dir", f"{dir_prefix}{env_name}")
        ...
        if not zip_files:
            print(
                f"Skipping {env_name}: No models found in {log_dir} matching prefix '{name_prefix}'"
            )
            continue
    ...
    print("=" * 120)
    print(
        f"{'Algo':<10} | {'Run ID':<10} | {'Environment':<20} | {'Mean Reward':<12} | {'Norm Reward':<12} | {'File'}"
    )
    print("-" * 120)
    ...
    for _, row in df_sorted.iterrows():
        print(
            f"{row['algo']:<10} | {row['run_id']:<10} | {row['env']:<20} | "
    ...
    summary = (
        df.groupby(["algo", "run_id"])["norm_reward"]
        .agg(["mean", "count"])
        .reset_index()
    )
    ...
    for _, row in summary.iterrows():
        print(
            f"{row['algo']:<10} | {row['run_id']:<20} | {row['mean']:>15.4f} | {row['count']:<5}"
        )
...
def main():
    ...
    if args.config_files:
        ...
        summary = run_evaluation_and_process(
        ...
        if os.path.exists(args.all_results_file):
            try:
                ...
                combined_df = pd.concat(
                    [existing_df, summary_to_save], ignore_index=True
                )
                updated_df = combined_df.drop_duplicates(
                    subset=["env", "algo", "run_id"], keep="last"
                )
            except Exception as e:
        ...
        if os.path.exists(args.all_results_file):
            ...
            print(
                f"Error: {args.all_results_file} does not exist and no config files provided."
            )
            exit(1)
    ...
    total_frppo_reward = updated_df[updated_df["algo"] == "FRPPO"][
        "avg_norm_reward"
    ].sum()
    ...
    env_max_scores = (
        updated_df.groupby(["env", "algo"])["avg_norm_reward"].max().unstack()
    )
